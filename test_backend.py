...
async def get_petition_signatures():
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT id, name, location, email, signed_at FROM petition_signatures ORDER BY signed_at DESC')
...
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "vault2k-admin-2024")
...
SITE_ROOT = os.path.join(WORKSPACE_ROOT, "artifacts/nba-vault")
BACKEND_ROOT = os.path.join(WORKSPACE_ROOT, "artifacts/nba-vault-backend")
...
EDITOR_TOOLS = [
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "File path relative to artifacts/nba-vault/. Examples: 'src/pages/LandingPage.jsx', 'index.html', 'src/index.css'. To read a backend file prefix with '../nba-vault-backend/', e.g. '../nba-vault-backend/server.py'"}
            },
            "required": ["path"]
        }
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "Directory path relative to artifacts/nba-vault/. Use '.' for site root, 'src' for source, 'src/pages' for pages, etc."}
            },
            "required": ["path"]
        }
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "File path relative to artifacts/nba-vault/. Use '../nba-vault-backend/server.py' for backend."},
    ...
    {
        "name": "run_command",
        "description": "Run a shell command from the workspace root. Use for: installing packages (pnpm --filter @workspace/nba-vault add <pkg>), build checks, or inspecting the environment.",
        "input_schema": {
            "type": "object",
            "properties": {
...
PROJECT LAYOUT:
- Frontend: React + Vite + Tailwind CSS → artifacts/nba-vault/
  ...
  - src/App.tsx                (routing)
- Backend: FastAPI Python → artifacts/nba-vault-backend/server.py
...
6. When installing a package: use `pnpm --filter @workspace/nba-vault add <pkg>` for frontend, `pip install <pkg>` for backend
...
def _build_site(log_fn):
    ...
    result = _subprocess.run(
        "pnpm --filter @workspace/nba-vault build",
...
- "file" is relative to workspace root (e.g. artifacts/nba-vault-backend/server.py)
...
async def doctor_lock_in():
    ...
    try:
        result = _sp.run(
            ["python3", "-m", "pytest", "artifacts/nba-vault-backend/test_backend.py", "-v", "--tb=short"],
            capture_output=True, text=True, timeout=120, cwd=WORKSPACE_ROOT
        )
        output = (result.stdout + result.stderr).strip()
        lines = output.split("\n")
        summary_line = next((l for l in reversed(lines) if "passed" in l or "failed" in l or "error" in l), "No summary found")
        passed = result.returncode == 0
        _log_event("doctor", "lock_in", f"{summary_line}", "ok" if passed else "error")
        return {"passed": passed, "summary": summary_line, "output": output[-4000:], "returncode": result.returncode}
    except Exception as e:
...
async def get_code_files():
    api_server_root = os.path.join(WORKSPACE_ROOT, "artifacts", "api-server")
    frontend = _walk_code(SITE_ROOT, "nba-vault")
    backend  = _walk_code(BACKEND_ROOT, "nba-vault-backend") + _walk_code(api_server_root, "api-server")
    return {"frontend": frontend, "backend": backend}

@api_router.get("/admin/code/frontend.zip")
async def download_frontend_zip():
    data = _make_zip_bytes([(SITE_ROOT, "nba-vault")])
...
async def download_backend_zip():
    ...
    data = _make_zip_bytes([
        (BACKEND_ROOT, "nba-vault-backend"),
        (api_server_root, "api-server"),
    ])
    return StreamingResponse(
        io.BytesIO(data), media_type="application/zip",
